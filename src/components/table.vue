      ...
      <table id="example" class="table ys-table table-striped table-bordered dataTable" style="width: 100%;" role="grid" aria-describedby="example_info">
        <thead v-if="columns.length">
          <tr role="row">
            ...
            <th v-for="column in columns" :key="'heading'+column.label" scope="col"  tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" :style="column.style">
              <div class="cell">
                <slot :name="'column('+column.label+')'" :item="column">
        ...
        <tbody v-if="data.length">
          <tr v-for="(row,index) in data" :cell="index" :key="'ys-datatable-row-'+index">
            <slot :row="row">
              ...
              <td v-for="column in columns" :key="'ys-datatable-row-column-'+column.label">
                <slot :name="column.label" :item="row">
                {{row[snake_case(column.label)]}}
                </slot>
              </td>
            </slot>
          </tr>
        ...
        <tfoot v-if="footer && columns.length">
          <tr >
            ...
            <th v-for="column in columns" :key="'row'+column.label" scope="col"  tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" :style="column.style">
